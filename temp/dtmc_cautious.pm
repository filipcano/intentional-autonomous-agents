...
)  &  (turn = 1) & (finished=0) & (car_x < street_length) & (crashed=0) & (!is_slippery) -> // Accelerate
...
)  &  (turn = 1) & (finished=0) & (car_x < street_length) & (crashed=0) & (allowed_to_brake) & (!is_slippery) -> //& (car_v > 0) -> // Brake
...
)  &  (turn = 1) & (finished=0) & (car_x < street_length) & (crashed=0) & (!allowed_to_brake) -> //& (car_v > 0) -> // Brake
...
)  &  (turn = 1) & (finished=0) & (car_x < street_length) & (crashed=0) & (allowed_to_noop)-> // Stays the same speed
noop0: (car_x' = min(street_length, car_x + max(0, car_v)))&(turn' = 2) +
noop1: (car_v' = max(0, car_v - 1))&(car_x' = min(street_length, car_x + max(0, car_v - 1)))&(turn' = 2);
...
)  &  (turn = 1) & (finished=0) & (car_x < street_length) & (crashed=0) & (!allowed_to_noop) & (!is_slippery)-> // Stays the same speed
acc2_prob: (car_v' = min(max_speed, car_v + 2))&(car_x' = min(street_length, car_x + min(max_speed, car_v + 2)))&(turn' = 2) +
acc1_prob: (car_v' = min(max_speed, car_v + 1))&(car_x' = min(street_length, car_x + min(max_speed, car_v + 1)))&(turn' = 2) +
acc0_prob: (car_x' = min(street_length, car_x + car_v + 0))&(turn' = 2);
